...
class Config:
    ...
    mat_path = '/Users/cizer/Downloads/taoliu/RESULTS/data'
    ...
    for stage_idx in range(2):
        ...
        for roi in cf.ROIs:
            ...
            for subj in subj_list[stage_idx]:
                ...
                path = os.path.join(cf.mat_path, subj, f'{subj}_trial_roi_{str(roi)}', subj, 'trial', str(roi))
                mom_voxel_list = []

                if roi % 2 == 0:
                    for num in range(1,27):
                        mom_voxel = sio.loadmat(path+f'/{subj}_voxel_{num}_l.mat')['momint_1']
                        data_filter = eeg_bp_filter(mom_voxel[:, :200], fs=100, freqb=freqb)
                        mom_voxel_list.append(data_filter)
                        del mom_voxel, data_filter
                else:
                    for num in range(1,27):
                        mom_voxel = sio.loadmat(path+f'/{subj}_voxel_{num}_r.mat')['momint_1']
                        data_filter = eeg_bp_filter(mom_voxel[:, :200], fs=100, freqb=freqb)
                        mom_voxel_list.append(data_filter)
                        del mom_voxel, data_filter
...
def main():
    cf = Config()
    subj_list = load_data(cf, 'AO')
    for f in cf.freqb:
        VAR_stage, VAR_diff = get_variance(cf, subj_list, f)
        visualize_result(cf, VAR_stage, VAR_diff, cf.save_path, cf.Paradigm[0], f)
