...
def minmax_normalize(data):
    """
    Perform Min-Max normalization to scale data to the range [0, 1].

    Parameters:
        data (array-like): Input data to normalize (1D or 2D array).

    Returns:
        normalized_data (numpy array): Data normalized to [0, 1].
    """
    data = np.array(data)
    min_val = np.min(data)
    max_val = np.max(data)
    ...
    if min_val == max_val:
        raise ValueError("Data has no variation (min and max are the same). Cannot normalize.")
