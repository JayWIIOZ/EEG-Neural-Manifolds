...
for num in range(len(CCA_score)):
    ...
    for dim in range(CCA_score_diff.shape[-1]):
        res = pearsonr(CCA_score_diff[:, dim], fma_diff)
        corr_list.append(res.correlation)
        p_list.append(res.pvalue)
    corr.append(corr_list)
    p_value.append(p_list)

# visualization
LABELs = ROIs_label = ['PreCG.L - PreCG.R','SMA.L - SMA.R','SPG.L - SPG.R','IPL.L - IPL.R']
fig,ax = plt.subplots(ncols=1)
for ii in range(len(p_value)):
    ax.plot(np.arange(1, len(p_value[ii])+1),np.array(p_value[ii]),label=LABELs[ii])
